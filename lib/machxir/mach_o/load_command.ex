defmodule Machxir.MachO.LoadCommand do
  import Bitwise
  alias Machxir.{ByteCrawler, Utils}
  ...
  defp parse_inner(cmd, cmdsize, pid, arch, nil, opt) do
    ...
    case cmd do
      ...
      0x04 -> LoadCommand.Thread.parse(pid, cmdsize)
      0x05 -> LoadCommand.Thread.parse(pid, cmdsize)
      ...
      0x10 -> LoadCommand.PreboundDylib.parse(pid, arch)
      ...
      0x1B -> LoadCommand.Uuid.parse(pid)
      ...
      0x24 -> LoadCommand.VersionMin.parse(pid)
      0x25 -> LoadCommand.VersionMin.parse(pid)
      ...
      0x2F -> LoadCommand.VersionMin.parse(pid)
      0x30 -> LoadCommand.VersionMin.parse(pid)
  ...
  defp parse_inner(cmd, _cmdsize, pid, arch, :req_dyld, opt) do
    case cmd do
      ...
      0x1C -> LoadCommand.Rpath.parse(pid, arch)
      0x1F -> LoadCommand.Dylib.parse(pid, arch, opt)
      0x22 -> LoadCommand.DyldInfo.parse(pid, opt)
      0x23 -> LoadCommand.Dylib.parse(pid, arch, opt)
  ...
  def format(cmd, nil) do
    case cmd do
      ...
      _ -> "INVALID (#{Utils.to_padded_hex32(cmd)})"
    end
  end
  ...
  def format(cmd, :req_dyld) do
    case cmd do
      ...
      0x35 -> "FILESET_ENTRY"
      _ -> "INVALID (#{Utils.to_padded_hex32(cmd)} found for LC_REQ_DYLD)"
    end
  end
end
